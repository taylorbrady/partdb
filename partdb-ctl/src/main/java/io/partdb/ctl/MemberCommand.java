...
final class MemberCommand {
    ...
    private static void printText(MemberListResponse r, PrintStream out) {
        out.printf("%-12s %-24s %-8s %-8s%n", "NODE ID", "ADDRESS", "ROLE", "STATUS");
        for (Member m : r.getMembersList()) {
            String status = m.getIsLeader() ? "leader" : (m.getIsSelf() ? "self" : "");
            out.printf("%-12s %-24s %-8s %-8s%n",
                m.getNodeId(),
                m.getAddress().isEmpty() ? "(unknown)" : m.getAddress(),
                m.getRole().name().toLowerCase(),
                status);
        }
    }

    private static void printJson(MemberListResponse r, PrintStream out) {
        out.print("{\"leaderId\":");
        out.print(r.getLeaderId().isEmpty() ? "null" : "\"" + r.getLeaderId() + "\"");
        out.print(",\"members\":[");
        var members = r.getMembersList();
        for (int i = 0; i < members.size(); i++) {
            Member m = members.get(i);
            if (i > 0) out.print(",");
            out.print("{\"nodeId\":\"" + m.getNodeId() + "\"");
            out.print(",\"address\":" + (m.getAddress().isEmpty() ? "null" : "\"" + m.getAddress() + "\""));
            out.print(",\"role\":\"" + m.getRole().name().toLowerCase() + "\"");
            out.print(",\"isLeader\":" + m.getIsLeader());
            out.print(",\"isSelf\":" + m.getIsSelf() + "}");
        }
        out.println("]}");
    }
    ...
    private static void printUsage(PrintStream out) {
        out.println("Usage: partdbctl member <subcommand> [options]");
        out.println();
    ...
    private static void printListUsage(PrintStream out) {
        out.println("Usage: partdbctl member list [options]");
        out.println();
