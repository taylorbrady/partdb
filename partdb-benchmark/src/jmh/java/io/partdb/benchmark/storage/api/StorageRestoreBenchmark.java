...
public class StorageRestoreBenchmark {
    ...
    @Benchmark
    public void restoreIntoNewStore(RestoreState state) {
        state.targetStore.replaceWith(state.checkpoint);
    }
    ...
    public static class RestoreState {
        ...
        private VersionedKeyValueStore targetStore;

        @Setup(Level.Trial)
        public void createCheckpoint() throws IOException {
            sourceDir = BenchmarkDirectories.createTempDirectory("partdb-restore-source");
            try (VersionedKeyValueStore sourceStore = VersionedKeyValueStore.open(sourceDir, config)) {
        ...
        public void openTargetStore() throws IOException {
            targetDir = BenchmarkDirectories.createTempDirectory("partdb-restore-target");
            targetStore = VersionedKeyValueStore.open(targetDir, config);
        }
